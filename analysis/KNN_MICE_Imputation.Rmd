...
for (s in 1:sim_number){
  ...
  if (type == "regression"){
    ...
    # Logistic Regression 
    logistic_regression <- glm(form_train, data = knn_impute_set, 
                                 family = "binomial")
    ...
    # Random Forest 
    rf_class_knn <- randomForest(form_train, data = knn_impute_set, 
    ...
    if (class_hyper$kernel == "linear"){
       svm_knn <- svm(form_train, data= knn_impute_set, kernel = "linear", 
    ...
    if (class_hyper$kernel == "polynomial"){
      ...
      svm_knn <- svm(form_train, data= knn_impute_set, kernel = "polynomial", 
    ...
    if (class_hyper$kernel == "radial"){
      ...
      svm_knn <- svm(form_train, data= knn_impute_set, kernel = "radial", 
    ...
    knn_impute_set[[response]] <- as.numeric(knn_impute_set[[response]]) - 1
    
    # Train GBM
    gbm_class_mice <- gbm(form_train, distribution = "bernoulli", data = knn_impute_set, 
